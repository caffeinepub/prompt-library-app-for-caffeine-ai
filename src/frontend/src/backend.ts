...
export class Backend implements backendInterface {
    ...
    async backendConnectivityCheck(): Promise<void> {
        if (this.processError) {
            try {
                const result = await this.actor.backendConnectivityCheck();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function from_candid_opt_n7(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_Prompt]): Prompt | null {
    return value.length === 0 ? null : value[0];
}
